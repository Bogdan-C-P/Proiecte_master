...
for i in range(len(labels_list)):
    #color_dict[labels_list[i] ] = tuple(color_map[i])
...
for i in range(10,20):
    ...
    img_path = training_image_paths[i]
    mask_path = training_annotation_paths[i]
    ...
    rgb_mask = np.zeros((224, 224,3), dtype=np.uint8)
    for unique_value, color_triplet in color_dict.items():
        pixels = mask == unique_value
        rgb_mask[pixels] = color_triplet
    ...
    rgb_pred = np.zeros((224, 224,3), dtype=np.uint8)
    for unique_value, color_triplet in color_dict.items():
        pixels = pred == unique_value
        rgb_pred[pixels] = color_triplet
